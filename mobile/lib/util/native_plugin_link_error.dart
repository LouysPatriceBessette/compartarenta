...
String nativePluginLinkErrorRecoveryMessage({String? languageCode}) {
  ...
  if (fr) {
    return 'Les plugins natifs (stockage local) ne sont pas chargés.\n\n'
        ...
        '2. Fermeture forcée de « Compartarenta (Dev) » (pas seulement retour arrière) '
        'et retrait des recents Android\n'
        '3. Désinstallez « Compartarenta (Dev) » (Réglages → Applications)\n'
        ...
        'Vérifiez le flavor dev (Compartarenta (Dev)), pas staging/prod.';
  }
  return 'Native plugins (local storage) are not loaded.\n\n'
      '1. Stop every melos / flutter command (Ctrl+C)\n'
      '2. Force-stop “Compartarenta (Dev)” (not just back) and remove it from '
      'Android recents\n'
      '3. Uninstall “Compartarenta (Dev)” (Settings → Apps)\n'
      ...
      'Use the dev flavor (Compartarenta (Dev)), not staging/prod.';
}
