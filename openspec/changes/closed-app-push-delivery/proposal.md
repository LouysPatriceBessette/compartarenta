...
- Move the relay envelope ingestion code path to fan out a best-effort wake push to all active tokens registered for the recipient routing id, with the payload limited to `{ v, kind, recipient }`. Multiple tokens per routing id are a transport artifact (refresh overlap), not product support for one user on several simultaneous devices.
