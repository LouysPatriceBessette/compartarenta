...
> copy, Settings timezone → relay upsert **2.2b**, operator docs **1.5**) are
> **implemented in the repo** for native (Android) paths. End-to-end field
> validation (activation → cron fire → FCM wake → notification) and relay cron
> claim integration test (**3.1**) remain **open**.
