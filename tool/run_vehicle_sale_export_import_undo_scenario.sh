#!/usr/bin/env bash
# Vehicle sale export → wipe → import → tap Défaire l'importation (single emulator).
#
# Phases 1–6 match run_vehicle_sale_export_import_scenario.sh. Phase 7 uses
# vehicle_sale_import_undo.yaml (same import steps through undo visible, then
# tap qa-vehicle-import-undo — observe product behavior; no edit/rename path).
#
# Usage:
#   ./tool/run_vehicle_sale_export_import_undo_scenario.sh
#   ./tool/run_vehicle_sale_export_import_undo_scenario.sh --skip-build --skip-install
...
echo "=== Phase 7 === device=${EMULATOR_SERIAL} Maestro import+undo flow=${FLOW_IMPORT}"
